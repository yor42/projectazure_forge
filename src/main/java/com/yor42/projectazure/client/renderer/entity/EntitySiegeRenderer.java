...
public class EntitySiegeRenderer extends GeoCompanionRenderer<EntitySiege>{
    ...
    @Override
    public void renderEarly(EntitySiege animatable, MatrixStack stackIn, float ticks, @Nullable MultiBufferSource renderTypeBuffer, @Nullable VertexConsumer  vertexBuilder, int packedLightIn, int packedOverlayIn, float red, float green, float blue, float partialTicks) {
    ...
    @Override
    public void render(@Nonnull EntitySiege entity, float entityYaw, float partialTicks, MatrixStack stack, MultiBufferSource bufferIn, int packedLightIn) {
        stack.pushPose();
    ...
    public void renderRecursively(GeoBone bone, MatrixStack stack, VertexConsumer  bufferIn, int packedLightIn, int packedOverlayIn, float red, float green, float blue, float alpha) {
        if (bone.getName().equals("itemOffHand")){
            ...
            stack.translate(-0.6F, 0.2F, 1.65F);
            ...
            stack.translate(xvalue, 0.2F, 1.65F);
            stack.scale(1.5F, 1.5F, 1.5F);
            if(!mainHandStack.isEmpty()){
                Minecraft.getInstance().getItemRenderer().renderStatic(mainHandStack, ItemCameraTransforms.TransformType.THIRD_PERSON_RIGHT_HAND, packedLightIn, packedOverlayIn, stack, this.rtb);
            }
