...
public class EntityRabu extends EntityGunUserBase{
    public EntityRabu(EntityType<? extends TamableAnimal> type, Level worldIn) {
        super(type, worldIn);
    }

    @Override
    public enums.GunClass getGunSpecialty() {
        return enums.GunClass.SG;
    }
    ...
    public static AttributeSupplier.Builder MutableAttribute()
    {
        return Mob.createMobAttributes()
                //Attribute
                .add(Attributes.MOVEMENT_SPEED, PAConfig.CONFIG.RabuMovementSpeed.get())
                .add(ForgeMod.SWIM_SPEED.get(), PAConfig.CONFIG.RabuSwimSpeed.get())
                .add(Attributes.MAX_HEALTH, PAConfig.CONFIG.RabuHealth.get())
                .add(Attributes.ATTACK_DAMAGE, PAConfig.CONFIG.RabuAttackDamage.get())
                ;
    }
