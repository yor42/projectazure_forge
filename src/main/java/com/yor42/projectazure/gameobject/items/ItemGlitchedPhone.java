...
public class ItemGlitchedPhone extends Item {
    ...
    @OnlyIn(Dist.CLIENT)
    private void openGUI(InteractionHand hand){
    ...
    public void addInformationAfterShift(ItemStack stack, @Nullable Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
        ...
        tooltip.add(new TranslatableComponent("item.projectazure.glitched_phone.tooltip1").withStyle(ChatFormatting.GRAY));
        tooltip.add(new TranslatableComponent("item.projectazure.glitched_phone.tooltip2").withStyle(ChatFormatting.GRAY));
    }
